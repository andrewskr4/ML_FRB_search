...
int main(int argc, char *argv[])
{
  ...
  int i;
	int sample_shift=0,sample_number=0,sample_smooth=1,sample_dm=5,sample_correction=0;  
  printf("opening file \n");
  ...
  for(i=0;i<argc;i++)
  {
		...
		if(strcmp(argv[i],"-s")==0)
    {
	  ...
	  {
		  i++;
	    sample_number= atoi(argv[i]);
    }

    if(strcmp(argv[i],"-sm")==0)
	  {
		  i++;
	    sample_smooth= atoi(argv[i]);
    }
    
		if(strcmp(argv[i],"-d")==0)
		{
		  i++;
			sample_dm= atoi(argv[i]);
	  }
  ...
  for(int iter=0;iter<10;iter++)
	{
		if(sample_dm<=5) correction = -1*(rand()%100);
		else correction = (rand()%100);
		 
	  dmshift(fch1, fch1+(foff*nchans), nchans, dm_value+sample_dm-5+((((float)(rand()%10))/10.0)-0.5), fch1, tsamp, shift);
	  dedisperse(dataOut, buffer, shift, nchans, num_use_samples-1);
	  for(int i=0; i<256*256*4; i++) plot[i]=0;
	   //printf("Entered the loop \n"); 
	   //for(int smooth=1;smooth<32;smooth=smooth*2)
	   {	   
			 //printf("%d \n",smooth); 
			     ...
			     for(int c=0; c<4; c++)
				   {
					   for(int s=0; s<1; s++)
					   {
						   for(int sm=0;sm<sample_smooth;sm++)
					     {
						     if(flags[4*channel+c]==0) plot[256*channel+sample] += dataOut[(num_use_samples-128*sample_shift-(sample_correction+correction)-(sample_smooth-1)*128)*nchans+(sample*sample_smooth+sm)*nchans+(4*channel+c)]; 
		  ...
		  sprintf(file_name,"pulse_%d.plt",10*sample_number+iter);
      ...
      temp= dm_value;
		  fwrite(&temp,sizeof(float),1,fpout);
		  temp= (float)sample_smooth;
		  fwrite(&temp,sizeof(float),1,fpout);
		  temp= (float)sample_shift;
			fwrite(&temp,sizeof(float),1,fpout);
			temp = float(sample_dm);
		  fwrite(&temp,sizeof(float),1,fpout);
		  temp = 1.0;
		  fwrite(&temp,sizeof(float),1,fpout);
