...
int main(int argc, char *argv[])
{
  ...
  int i;
	int sample_shift=0,sample_correction=0,sample_number=0;  
  //printf("opening file \n");
    ...
    for(int dm_shift=-5;dm_shift<6;dm_shift++)
	  {
	    ...
	    //printf("Entered the loop \n"); 
	     ...
	     {	   
			       ...
			       for(int c=0; c<4; c++)
				     {
					     for(int s=0; s<1; s++)
					     {
					       ...
					       {
						       int correction = 256+smooth*256;
								   if(flags[4*channel+c]==0) plot[256*channel+sample] += dataOut[(num_use_samples-128*sample_shift-(sample_correction+correction)-(smooth-1)*128)*nchans+(sample*smooth+sm)*nchans+(4*channel+c)];
		    ...
		    sprintf(file_name,"rfi_%d.plt",sample_number*440+p);
