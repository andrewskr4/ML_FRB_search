//***************************************************************************
// ********* gen_rfi.cpp *********************************
// Code to generate dynamic spectra files from filterbank data
//
// //***************************************************************************
...
// normalize every dynamic spectra generated. 
// This is crucial as it will takeout most of the instrumental effectss
...
int main(int argc, char *argv[])
{
  ...
  read_header(fpin); //reads in filterbank header
  ...
  fileSize -= headerSize;
	...
	unsigned char *dataOut = new unsigned char[nchans*8192*3+2048*3];
	float *avg_data = new float[nchans*10240];
	dm_value=200.0;
  ...
  srand(time(NULL));
    ...
    fflush(stdout);
    dm_value = 100+rand()%400;
		dmshift(fch1, fch1+(foff*nchans), nchans, dm_value, fch1, tsamp, shift);
		dedisperse(dataOut, buffer, shift, nchans, 8192*3+2048*3);
		
		for(int i=0; i<nchans*10240; i++) avg_data[i] = 0.0;
		...
		for(int iter=0; iter<64; iter++)
		{
			...
			for(int smooth=1;smooth<16;smooth=smooth*2)
			{
		    ...
		    normalize(plot);  
			  ...
			  sprintf(file_name,"rfi_%d.plt",start_number+p);
