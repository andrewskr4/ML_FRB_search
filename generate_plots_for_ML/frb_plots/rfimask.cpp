...
int main(int argc, char *argv[])
{
  ...
  srand(time(NULL));
    ...
    for(int i=0; i<256*256*4; i++) plot[i]=0;
			  ...
			  for(int c=0; c<4; c++)
				{
					for(int s=0; s<1; s++)
					{
						if(flags[4*channel+c]==0) plot[256*channel+sample] += dataOut[(num_use_samples-p*128)*nchans+(sample)*nchans+(4*channel+c)];
		...
		sprintf(file_name,"pulse_%d_%d_%d.plt",event_int,ibeam,p);
