...
def plot_sample(filename,figname, npix):
    ...
    x = x.reshape(256,256)
    #print(x)
    xpix = [[0 for p in range(npix)] for q in range(npix)]
    div = 256/npix
    ...
    for i in range(0,255):
        for j in range(0,255):
            temp+=x[i][j]
            if (j+1)%div == 0:
                xpix[int((i+1)/div)][int((j+1)/div)] = temp
                temp = 0
    #print(x128)
    ...
    ax1.grid(False)
    ax1.set_xticks([0,(npix/4)-1,(2*npix/4)-1,(3*npix/4)-1,npix-1])
    ax1.set_yticks([0,(npix/4)-1,(2*npix/4)-1,(3*npix/4)-1,npix-1])
    ax2.set_xticks([0,(npix/4)-1,(2*npix/4)-1,(3*npix/4)-1,npix-1])
...
plot_sample(str(sys.argv[1]),str(sys.argv[2])+str('px_')+str(outname), int(sys.argv[2]))
